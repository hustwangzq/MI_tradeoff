function ch = SIM_build_channels(params, L, mode, common)
...
if nargin < 4 || isempty(common)                   % Backward compatibility if no common links are supplied.
    common = SIM_build_common_links(params);        % Generate the common outer links once locally.
end
...
G = SIM_diffraction_matrix(layerPos{1}, bsPos, lambda, d^2); % G_st: BS-to-first-layer near-field matrix.

Omega = cell(L,1);                                 % Inter-layer propagation matrices Omega_ell.
for ell = 2:L                                      % For ell>=2, Omega{ell} maps layer ell-1 to layer ell.
    Omega{ell} = SIM_diffraction_matrix(layerPos{ell}, layerPos{ell-1}, lambda, d^2); % Near-field inter-layer matrix.
end
